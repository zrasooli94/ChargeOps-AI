...
import { createRequestStore as createRequestStoreFromInputs, createRequestStoreForRender } from '../async-storage/request-store';
...
import { getDevValidationWorker } from './dev-validation-worker-globals';
...
import { getRequestInsightsIdentity, runWithRequestInsightsIdentity } from '../lib/trace/request-insights-identity';
...
import { appendMutableCookies, RequestCookiesAdapter } from '../web/spec-extension/adapters/request-cookies';
...
import { beginDevValidation, yieldToForegroundRequest } from './dev-validation-scheduler';
...
import { ReactServerPrerenderResult, createReactServerPrerenderResult, ReactServerResult, ReplayableNodeStream, createReactServerPrerenderResultFromRender } from './app-render-prerender-utils';
...
import { createResponseVaryParamsAccumulator, finishAccumulatingVaryParams, getMetadataVaryParamsAccumulator, getRootParamsVaryParamsAccumulator } from './vary-params';
import { getTracedMetadata } from '../lib/trace/utils';
import { InvariantError } from '../../shared/lib/invariant-error';
import { StaleTimeIterable, createSelectStaleTime, trackStaleTime } from './stale-time';
...
import { getNextStage, isAdvanceableRenderStage, RENDER_STAGE_ADVANCE_ORDER, RenderStage, StagedRenderingController, SyncIOMode } from './staged-rendering';
import { anySegmentHasPartialPrefetchingEnabled, isPageAllowedToBlock, anySegmentNeedsInstantValidationInDev, anySegmentNeedsInstantValidationInBuild, resolveInstantConfigSamplesForPage } from './instant-validation/instant-config';
import { warnOnce } from '../../shared/lib/utils/warn-once';
import { createWebDebugChannel, createNodeDebugChannel } from './debug-channel-server';
import { createNodeStreamFromChunks, createNodeStreamWithLateRelease } from './instant-validation/stream-utils';
 ...
 */ async function generateDynamicFlightRenderResult(req, ctx, requestStore, options) {
    ...
    const requestAbortSignal = process.env.__NEXT_DEV_SERVER && renderOpts.experimental.serverComponentsHmrCancellation === true && requestStore.isHmrRefresh === true && isNodeNextResponse(ctx.res) ? signalFromNodeResponse(ctx.res.originalResponse) : undefined;
 ...
 */ async function generateStagedDynamicFlightRenderResultNode(req, ctx, requestStore) {
    ...
    const staticStageByteLengthDeferred = createPromiseWithResolvers();
    ...
    if (Boolean(renderOpts.partialPrefetching) || await anySegmentHasPartialPrefetchingEnabled(loaderTree)) {
 ...
 */ async function generateDynamicFlightRenderResultWithStagesInDev(req, ctx, initialRequestStore, createRequestStore, fallbackParams) {
    ...
    const shouldValidate = !ctx.isPrefetch && !isBypassingCachesInDev(initialRequestStore, workStore) && (initialRequestStore.isHmrRefresh === true || await anySegmentNeedsInstantValidationInDev(loaderTree));
    ...
    !isBypassingCachesInDev(initialRequestStore, workStore)) {
        ...
        // render" button.
        const requestAbortSignal = renderOpts.experimental.serverComponentsHmrCancellation === true && initialRequestStore.isHmrRefresh === true && isNodeNextResponse(ctx.res) ? signalFromNodeResponse(ctx.res.originalResponse) : undefined;
...
async function generateRuntimePrefetchResult(req, ctx, requestStore, isShellPrefetch) {
    ...
    const debugChannel = setReactDebugChannel ? createWebDebugChannel() : undefined;
    if (debugChannel && setReactDebugChannel) {
        setReactDebugChannel(debugChannel.clientSide, htmlRequestId, requestId);
    }
...
async function finalRuntimeServerPrerender(mode, ctx, getPayload, resumeDataCache, rootParams, headers, cookies, draftMode, onError, staleTimeIterable, debugChannel) {
    ...
    const finalStageController = new StagedRenderingController({
        ...
        finalStage: mode.type === 'session-shell-only' ? RenderStage.ShellRuntime : RenderStage.Runtime
    });
   ...
   * */ const checkUnexpectedAbort = ()=>{
    ...
    await runInSequentialTasks(async ()=>{
        finalStageController.advanceStage(RenderStage.ShellStatic);
        let stream = workUnitAsyncStorage.run(finalServerPrerenderStore, ComponentMod.renderToReadableStream, finalRSCPayload, clientModules, {
            filterStackFrame,
            onError,
        ...
        const didSessionDataUnblockNewContent = stageByteLengths[RenderStage.ShellRuntime] > stageByteLengths[RenderStage.Static];
        ...
        if (mode.type === 'rewindable-session-shell') {
            ...
            const didLinkDataUnblockNewContent = stageByteLengths[RenderStage.Runtime] > stageByteLengths[RenderStage.ShellRuntime];
            mode.shellByteLengthDeferred.resolve(didLinkDataUnblockNewContent ? stageByteLengths[RenderStage.ShellRuntime] : null);
        }
        staleTimeIterable.close();
        finishAccumulatingVaryParams(varyParamsAccumulator);
    }, ()=>{
        ...
        workUnitAsyncStorage.run(finalServerPrerenderStore, finalServerController.abort.bind(finalServerController));
    });
    const result = {
        prelude: new ReactServerPrerenderResult(prependIsPartialByteToChunks(collectedChunks.prerenderChunks, resultIsPartial)).consumeAsStream()
    };
    return {
        result,
...
async function getRSCPayload(tree, ctx, options) {
    ...
    const initialTree = await createFlightRouterStateFromLoaderTree(tree, hints, prefetchInliningEnabled, ctx.renderOpts.cacheComponents, ctx.renderOpts.partialPrefetching, workStore.isStaticGeneration, ctx.renderOpts.isBuildTimePrerendering ?? false, getDynamicParamFromSegment, query);
    const serveStreamingMetadata = !!ctx.renderOpts.serveStreamingMetadata;
...
async function getErrorRSCPayload(tree, ctx, ssrError, errorType, shouldRenderMetadataAndViewport) {
    ...
    const initialTree = await createFlightRouterStateFromLoaderTree(tree, errorHints, errorPrefetchInliningEnabled, ctx.renderOpts.cacheComponents, ctx.renderOpts.partialPrefetching, workStore.isStaticGeneration, ctx.renderOpts.isBuildTimePrerendering ?? false, getDynamicParamFromSegment, query);
    let err = undefined;
    if (ssrError) {
...
async function renderToHTMLOrFlightImpl(req, res, url, pagePath, query, renderOpts, workStore, parsedRequestHeaders, postponedState, serverComponentsHmrCache, sharedContext, interpolatedParams, fallbackRouteParams) {
    ...
    let requestId;
    let htmlRequestId;
    const requestInsightsIdentity = process.env.__NEXT_REQUEST_INSIGHTS ? getRequestInsightsIdentity() : undefined;
    ...
    if (isStaticGeneration) {
        ...
        const createRequestStore = createRequestStoreForRender.bind(null, req, res, url, rootParams, implicitTags, renderOpts.onUpdateCookies, renderOpts.previewProps, isHmrRefresh, serverComponentsHmrCache, renderResumeDataCache, fallbackParams, hmrRefreshHash);
...
async function renderToStream(requestStore, req, res, ctx, tree, formState, postponedState, metadata, createRequestStore, fallbackParams) {
    ...
    if (process.env.__NEXT_DEV_SERVER) {
        ...
        bootstrapScriptContent = getTurbopackChunkGroupBootstrap(buildManifest.pagesChunkGroupBootstrapParams, buildManifest.chunkLoadingGlobal, [
            page
        ]);
    }
    ...
    if (ctx.renderOpts.experimental.exposeTestingApi) {
        bootstrapScriptContent = (bootstrapScriptContent ? `${bootstrapScriptContent};` : '') + await getInstantTestBootstrapScriptContent();
    }
    ...
    return getTracer().withSpan(renderSpan, async ()=>{
        ...
        try {
            ...
            cacheComponents) {
                ...
                !isBypassingCachesInDev(requestStore, workStore)) {
                    ...
                    if (returnedDebugChannel) {
                        debugChannelClientStream = new ReplayableNodeStream(returnedDebugChannel.clientSide.readable);
                    }
                } else {
                    ...
                    reactServerResult = new ReactServerResult(serverStream);
                    if (debugChannel) {
                        debugChannelClientStream = new ReplayableNodeStream(debugChannel.clientSide.readable);
                    }
                }
            ...
            if (process.env.__NEXT_USE_NODE_STREAMS) {
                ...
                const { stream: htmlStream, allReady } = await getTracer().trace(AppRenderSpan.renderToNodeFizzStream, ()=>workUnitAsyncStorage.run(requestStore, renderToNodeFizzStream, appElement, fizzOptions, {
        ...
        // MARK: renderToStream errorRecovery
            ...
            const [errorPreinitScripts, errorBootstrapScript] = getRequiredScripts(buildManifest, assetPrefix, crossOrigin, subresourceIntegrityManifest, getAssetQueryString(ctx, false), nonce, UNDERSCORE_NOT_FOUND_ROUTE_ENTRY);
            let errorBootstrapScriptContent;
            if (process.env.__NEXT_DEV_SERVER) {
                errorBootstrapScriptContent = bootstrapScriptContent;
            } else if (buildManifest.pagesChunkGroupBootstrapParams && buildManifest.chunkLoadingGlobal) {
                errorBootstrapScriptContent = getTurbopackChunkGroupBootstrap(buildManifest.pagesChunkGroupBootstrapParams, buildManifest.chunkLoadingGlobal, [
            ...
            if (process.env.__NEXT_USE_NODE_STREAMS) {
                ...
                try {
                    errorRSCPayload = await workUnitAsyncStorage.run(requestStore, getErrorRSCPayload, tree, ctx, reactServerErrorsByDigest.has(err.digest) ? null : err, errorType, // Normal error rendering should include the error payload head.
                    true);
 ...
 */ function runDevValidationInBackground(prefetchMode, navigationKind, result, requestStore, validationDebugChannel, ctx, fallbackRouteParams, prerenderResumeDataCache, getDevRenderDidError, createRequestStore, getPayload, onError, validationGeneration) {
    ...
    void consoleAsyncStorage.run({
        ...
        if (isNodeNextResponse(ctx.res) && !await waitForResponseToFinish(ctx.res.originalResponse)) {
        ...
        return runInstantInsightsWithTracing(ctx, async (runSpan)=>{
            ...
            const [instantInputs, staticInputs] = await runSpan(AppRenderSpan.instantInsightsPrepareValidation, 'Prepare validation inputs', async ()=>{
            ...
            return runSpan(AppRenderSpan.instantInsightsRunValidation, 'Run validation', async ()=>{
                ...
                if (devValidationWorker) {
                    const snapshot = await buildDevValidationSnapshot(ctx, instantInputs, staticInputs, prefetchMode, fallbackRouteParams, devRenderDidError);
                    ...
                    if (chunks && !validationAbortSignal.aborted) {
                        ...
                        sendErrorsToBrowser(createNodeStreamFromChunks(chunks), ctx.htmlRequestId);
                    }
                } else {
    ...
    .catch((err)=>{
        ...
        console.error(Object.defineProperty(new InvariantError('An unexpected error occurred during validation', {
            cause: err
        }), "__NEXT_ERROR_CODE", {
            value: "E1393",
            enumerable: false,
...
async function prepareValidationInputsInPartialPrefetching(navigationKind, requestStore, ctx, prerenderResumeDataCache, createRequestStore, getPayload, onError, inputsFromNavigation, validationAbortSignal) {
    ...
    const areStagesCompatible = !hasNonRootStaticParams(ctx.interpolatedParams, requestStore.rootParams, requestStore.fallbackParams);
...
export var PrefetchingMode = /*#__PURE__*/ function(PrefetchingMode) {
...
function navigationHasAppShell(navigationKind) {
    ...
    return navigationKind.type === 'prefetched-client' && navigationKind.prefetchStage === RenderStage.ShellRuntime;
}
/**
 ...
 */ async function streamStagedRenderInDev({ ctx, requestStore, rscPayload, stageController, cacheSignal, environmentName, onError, debugChannel, navigationKind, requestAbortSignal }) {
    ...
    const revealAfter = createPromiseWithResolvers();
    ...
    const stagesAdvanced = runInSequentialTasks(()=>{
        ...
        const replayable = new ReplayableNodeStream(workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ComponentMod, rscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
    ...
    }, ()=>checkCacheMissAndAdvance(RenderStage.Static), ()=>checkReveal(RenderStage.Static), ()=>checkCacheMissAndAdvance(RenderStage.ShellRuntime), ()=>checkReveal(RenderStage.ShellRuntime), ()=>checkCacheMissAndAdvance(RenderStage.Runtime), ()=>checkReveal(RenderStage.Runtime), ()=>{
...
function getStageEndTimes(stageController) {
    return {
        ...
        [RenderStage.ShellRuntime]: stageController.getStageEndTime(RenderStage.ShellRuntime),
...
async function renderWithWarmCachesForValidationInDev(ctx, createRequestStore, getPayload, onError, prerenderResumeDataCache, prefetchMode, shouldRenderWithAppShell, validationAbortSignal) {
    ...
    const requestStore = createRequestStore();
    requestStore.resumeDataCache = createRenderResumeDataCache(prerenderResumeDataCache);
    requestStore.stagedRendering = stageController;
    requestStore.needsSessionShell = shouldRenderWithAppShell;
    ...
    const accumulatedChunks = await runInSequentialTasks(()=>{
        stageController.advanceStage(RenderStage.ShellStatic);
        startTime = performance.now() + performance.timeOrigin;
        const sourceStream = workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ComponentMod, rscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
    ...
    }, ()=>stageController.advanceStage(RenderStage.Static), ()=>stageController.advanceStage(RenderStage.ShellRuntime), ()=>stageController.advanceStage(RenderStage.Runtime), ()=>stageController.advanceStage(RenderStage.Dynamic));
    const syncInterruptReason = stageController.getSyncInterruptReason();
...
async function prerenderWithWarmCachesForStaticValidationInDev(ctx, createRequestStore, getPayload, onError, prerenderResumeDataCache, validationAbortSignal) {
    ...
    const collectChunk = (chunk)=>{
        ...
        const stage = finalReactController.signal.aborted ? RenderStage.Dynamic : stageController.currentStage;
    ...
    }, ()=>stageController.advanceStage(RenderStage.Static), ()=>stageController.advanceStage(RenderStage.ShellRuntime), ()=>stageController.advanceStage(RenderStage.Runtime), ()=>{
 ...
 * */ function abortInRenderContext(workUnitStore, controller, reason) {
    ...
    workUnitAsyncStorage.run(workUnitStore, reason ? controller.abort.bind(controller, reason) : controller.abort.bind(controller));
}
/**
 ...
 */ async function stagedRenderWithCachesInDev({ prefetchMode, ctx, requestStore, createRequestStore, getPayload, onError, shouldValidate, fallbackRouteParams, getDevRenderDidError, navigationKind, requestAbortSignal }) {
    const validationGeneration = shouldValidate ? beginDevValidation(ctx.htmlRequestId) : undefined;
    try {
        ...
        if (validationGeneration !== undefined && debugChannel) {
            const debugChannelReplay = new ReplayableNodeStream(debugChannel.clientSide.readable);
 ...
 */ export function toValidationRenderContext(ctx) {
    return {
 ...
 */ function toDevValidationInputs(serialized, requestStore) {
    return {
        ...
        requestStore,
        debugChannelClient: serialized.debugChunks ? createNodeStreamFromChunks(serialized.debugChunks) : undefined
    };
}
/**
 ...
 * to deliver.
 */ export async function runValidationInDevFromSnapshot(message, componentMod, abortSignal) {
    ...
    return workAsyncStorage.run(workStore, runValidationInDev, message.prefetchMode, instantInputs, staticInputs, ctx, fallbackRouteParams, message.devRenderDidError, abortSignal);
}
/**
...
async function validateStaticShell(inputs, ctx, rootParams, fallbackRouteParams, debugChunks, hmrRefreshHash, validationAbortSignal) {
    ...
    const runtimeResult = await validateStagedShell(runtimeChunks, dynamicChunks, debugChunks, stageEndTimes[RenderStage.Runtime], rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, trackDynamicHoleInRuntimeShell, validationAbortSignal);
    ...
    const staticResult = await validateStagedShell(staticChunks, dynamicChunks, debugChunks, stageEndTimes[RenderStage.Static], rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, trackDynamicHoleInStaticShell, validationAbortSignal);
...
async function warmupClientModulesForStagedValidation(storeType, partialServerChunks, allServerChunks, rootParams, fallbackRouteParams, ctx, validationSamples, validationSampleTracking, validationAbortSignal) {
    ...
    const serverStream = createNodeStreamWithLateRelease(partialServerChunks, allServerChunks, initialClientReactSignal);
    ...
    trackPendingModules(cacheSignal);
    await cacheSignal.cacheReady();
    workUnitAsyncStorage.run(initialClientPrerenderStore, initialClientReactController.abort.bind(initialClientReactController));
}
async function validateStagedShell(stageChunks, allServerChunks, debugChunks, debugEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, trackDynamicHole, validationAbortSignal) {
    ...
    const serverStream = createNodeStreamWithLateRelease(stageChunks, allServerChunks, clientReactSignal);
    const debugChannelClient = debugChunks ? createNodeStreamWithLateRelease(debugChunks, debugChunks, clientReactSignal) : undefined;
    try {
        let { prelude: unprocessedPrelude } = await runInSequentialTasks(()=>{
            ...
            /*#__PURE__*/ _jsx(App, {
                ...
                onError: (err, errorInfo)=>{
                    if (isPrerenderInterruptedError(err) || clientReactSignal.aborted) {
            ...
            workUnitAsyncStorage.run(finalClientPrerenderStore, clientReactController.abort.bind(clientReactController));
        });
    ...
    async function validateAtDepthImpl(depth, groupDepthForValidation, previousBoundaryState) {
        ...
        switch(prefetchKind){
            case ValidationPrefetchKind.Shell:
                {
                    dynamicHoleKind = payloadResult.hasAmbiguousErrors ? DynamicHoleKind.Link : DynamicHoleKind.Dynamic;
                    break;
                }
            case ValidationPrefetchKind.LegacySpeculative:
                {
                    dynamicHoleKind = payloadResult.hasAmbiguousErrors ? DynamicHoleKind.Runtime : DynamicHoleKind.Dynamic;
                    break;
                }
        }
        let result;
        try {
            const { prelude: unprocessedPrelude } = await runInSequentialTasks(()=>{
                ...
                /*#__PURE__*/ _jsx(App, {
                    ...
                    onError: (err, errorInfo)=>{
                        if (isPrerenderInterruptedError(err) || reactSignal.aborted) {
                            const componentStack = errorInfo.componentStack;
                            if (typeof componentStack === 'string') {
                                trackDynamicHoleInNavigation(err, workStore, componentStack, instantValidationState, clientDynamicTracking, dynamicHoleKind, boundaryState);
                            }
                            return;
                ...
                workUnitAsyncStorage.run(prerenderStore, reactController.abort.bind(reactController));
            });
            const { preludeIsEmpty } = await processPreludeOp(unprocessedPrelude);
        ...
        if (previousBoundaryState === null && payloadResult.hasAmbiguousErrors) {
            ...
            if (validationAbortSignal !== undefined && !await yieldToForegroundRequest(validationAbortSignal)) {
                return [];
            }
 ...
 */ async function renderWithRestartOnCacheMissInValidation(prefetchMode, ctx, initialRequestStore, createRequestStore, getPayload, createOnError, prefilledDataCache) {
    ...
    await cacheSignal.cacheReady();
    workUnitAsyncStorage.run(requestStore, initialReactController.abort.bind(initialReactController));
    //===============================================
    ...
    }, ()=>finalStageController.advanceStage(RenderStage.Static), ()=>finalStageController.advanceStage(RenderStage.ShellRuntime), ()=>finalStageController.advanceStage(RenderStage.Runtime), ()=>finalStageController.advanceStage(RenderStage.Dynamic));
    return {
...
async function validateInstantConfigsInBuild(ctx, prefilledDataCache) {
    ...
    if (process.env.__NEXT_TEST_MODE && process.env.NEXT_TEST_LOG_VALIDATION) {
        ...
        console.error(formatValidationEvent({
            type: 'validation_start',
            requestId,
            url: route
        }));
...
async function validateInstantConfigInBuildWithSample(outerCtx, sample, allPossibleFallbackRouteParams, prefilledDataCache) {
    ...
    return workAsyncStorage.run(workStore, async ()=>{
        ...
        const { accumulatedChunksPromise, startTime, stageController, requestStore: finalServerStore } = await renderWithRestartOnCacheMissInValidation(prefetchMode, validationCtx, createRequestStore(), createRequestStore, (requestStore)=>workUnitAsyncStorage.run(requestStore, getRSCPayload, loaderTree, validationCtx, {
                is404: false
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    // @TODO make it default empty string on renderOpts and get rid of it from ctx
    assetPrefix, crossOrigin, subresourceIntegrityManifest, getAssetQueryString(ctx, true), nonce, page);
    let bootstrapScriptContent = buildManifest.pagesChunkGroupBootstrapParams && buildManifest.chunkLoadingGlobal ? getTurbopackChunkGroupBootstrap(buildManifest.pagesChunkGroupBootstrapParams, buildManifest.chunkLoadingGlobal, [
        page
    ]) : undefined;
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            // resume data cache.
            const resumeDataCache = renderOpts.renderResumeDataCache ?? createPrerenderResumeDataCache();
            ...
            // from both.
            const runtimeDataAccessed = createPromiseWithResolvers();
            ...
            const shellByteLengthDeferred = createPromiseWithResolvers();
            const finalServerPayload = await workUnitAsyncStorage.run(finalServerPayloadPrerenderStore, getRSCPayload, tree, ctx, {
                is404: res.statusCode === 404,
                shellByteLengthPromise: shellByteLengthDeferred.promise
            });
       ...
       * */ const checkUnexpectedAbort = ()=>{
            ...
            await runInSequentialTasks(async ()=>{
                ...
                finalStageController.advanceStage(RenderStage.ShellStatic);
                let stream = workUnitAsyncStorage.run(finalServerPrerenderStore, ComponentMod.renderToReadableStream, finalServerPayload, clientModules, {
                    filterStackFrame,
                    onError: (err)=>{
                ...
                workUnitAsyncStorage.run(finalServerPrerenderStore, finalServerReactController.abort.bind(finalServerReactController));
            });
            ...
            let { prelude: unprocessedPrelude, postponed } = await runInSequentialTasks(()=>{
                const stream = process.env.NODE_ENV === 'development' && collectedChunks.allChunks ? createNodeStreamWithLateRelease(collectedChunks.prerenderChunks, collectedChunks.allChunks, finalClientReactController.signal) : reactServerResult.asUnclosingStream();
                ...
                /*#__PURE__*/ _jsx(App, {
                    ...
                    onError: (err, errorInfo)=>{
                        if (isPrerenderInterruptedError(err) || finalClientReactController.signal.aborted) {
                            const componentStack = errorInfo.componentStack;
                            if (typeof componentStack === 'string') {
                                trackAllowedDynamicAccess(err, workStore, componentStack, dynamicValidation, clientDynamicTracking);
                            }
                            return;
                        }
                ...
                workUnitAsyncStorage.run(finalClientPrerenderStore, finalClientReactController.abort.bind(finalClientReactController));
            });
            ...
            const RSCPayload = await workUnitAsyncStorage.run(pprReactServerPrerenderStore, getRSCPayload, tree, ctx, {
                is404: res.statusCode === 404
            });
            let reactServerResult;
            reactServerResult = reactServerPrerenderResult = await createReactServerPrerenderResultFromRender(workUnitAsyncStorage.run(pprReactServerPrerenderStore, renderFlightStream, ComponentMod, RSCPayload, clientModules, {
                filterStackFrame,
                onError: serverComponentsErrorHandler
            }));
       ...
       */ // First we check if we have any dynamic holes in our HTML prerender
        ...
        const [errorPreinitScripts, errorBootstrapScript] = getRequiredScripts(buildManifest, assetPrefix, crossOrigin, subresourceIntegrityManifest, getAssetQueryString(ctx, false), nonce, UNDERSCORE_NOT_FOUND_ROUTE_ENTRY);
        const errorBootstrapScriptContent = buildManifest.pagesChunkGroupBootstrapParams && buildManifest.chunkLoadingGlobal ? getTurbopackChunkGroupBootstrap(buildManifest.pagesChunkGroupBootstrapParams, buildManifest.chunkLoadingGlobal, [
            UNDERSCORE_NOT_FOUND_ROUTE_ENTRY
        ]) : undefined;
        if (cacheComponents) {
            ...
            if (originalFlightPrerenderResult === null) {
                throw Object.defineProperty(new InvariantError('Cache Components error recovery expected an original Flight prerender result'), "__NEXT_ERROR_CODE", {
                    value: "E1283",
                    enumerable: false,
                    configurable: true
                });
            ...
            if (originalFlightPrerenderResultIsDynamic === null) {
                throw Object.defineProperty(new InvariantError('Cache Components error recovery expected to know whether the original Flight prerender result was dynamic'), "__NEXT_ERROR_CODE", {
                    value: "E1282",
                    enumerable: false,
                    configurable: true
                });
            ...
            if (originalResumeDataCache === null) {
                throw Object.defineProperty(new InvariantError('Cache Components error recovery expected an original resume data cache'), "__NEXT_ERROR_CODE", {
                    value: "E1285",
                    enumerable: false,
                    configurable: true
                });
            ...
            if (originalPrerenderStore === null) {
                throw Object.defineProperty(new InvariantError('Cache Components error recovery expected an original prerender store'), "__NEXT_ERROR_CODE", {
                    value: "E1284",
                    enumerable: false,
                    configurable: true
                });
            ...
            const errorServerDynamicTracking = createDynamicTrackingState(isDebugDynamicAccesses);
            ...
            const errorRSCPayload = await workUnitAsyncStorage.run(errorPrerenderStore, getErrorRSCPayload, tree, ctx, reactServerErrorsByDigest.has(err.digest) ? undefined : err, errorType, // The recovery shell only bootstraps the original Flight data. Avoid
            // blocking that shell on error-page metadata or viewport.
            false);
            const errorServerResult = await createReactServerPrerenderResult(runInSequentialTasks(async ()=>{
                const pendingErrorServerResult = workUnitAsyncStorage.run(errorPrerenderStore, getServerPrerender(ComponentMod), errorRSCPayload, clientModules, {
                    filterStackFrame,
                ...
                if (!errorServerReactController.signal.aborted) {
                    workUnitAsyncStorage.run(errorPrerenderStore, errorServerReactController.abort.bind(errorServerReactController));
                }
            }));
            try {
                const errorClientReactController = new AbortController();
                const errorClientRenderController = new AbortController();
                const errorClientDynamicTracking = createDynamicTrackingState(isDebugDynamicAccesses);
                ...
                const { prelude: unprocessedErrorHtmlStream, postponed: errorPostponed } = await runInSequentialTasks(()=>{
                    const pendingErrorHtmlResult = workUnitAsyncStorage.run(errorClientPrerenderStore, getClientPrerender, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
                    /*#__PURE__*/ _jsx(ErrorApp, {
                        ...
                        onError: (clientError, errorInfo)=>{
                            if (isPrerenderInterruptedError(clientError) || errorClientReactController.signal.aborted) {
                                const componentStack = errorInfo.componentStack;
                                if (typeof componentStack === 'string') {
                                    trackAllowedDynamicAccess(clientError, workStore, componentStack, errorDynamicValidation, errorClientDynamicTracking);
                                }
                                return;
                            }
                    ...
                    workUnitAsyncStorage.run(errorClientPrerenderStore, errorClientReactController.abort.bind(errorClientReactController));
                });
                metadata.hasPendingUi = errorPostponed != null;
                const { prelude, preludeIsEmpty } = await processPreludeOp(unprocessedErrorHtmlStream);
                if (preludeIsEmpty) {
                    console.error(`Route "${workStore.route}" did not produce a static shell while rendering its error page.`);
                    throwIfDisallowedDynamic(workStore, PreludeState.Empty, errorDynamicValidation, errorServerDynamicTracking, false);
                    throw new StaticGenBailoutError();
                }
                const getServerInsertedHTML = makeGetServerInsertedHTML({
                    polyfills,
                ...
                if (originalFlightPrerenderResultIsDynamic) {
                    metadata.postponed = await getDynamicDataPostponedState(originalResumeDataCache, cacheComponents);
                    ...
                    return {
                        ...
                        dynamicAccess: consumeDynamicAccess(errorServerDynamicTracking, errorClientDynamicTracking),
                        ...
                        renderResumeDataCache: createRenderResumeDataCache(originalResumeDataCache)
                    };
                    ...
                    const foreverStream = createPendingStream();
                    const resumePrelude = await workUnitAsyncStorage.run(errorPrerenderStore, resumeAndAbort, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
                    /*#__PURE__*/ _jsx(ErrorApp, {
                        reactServerStream: foreverStream,
                        ServerInsertedHTMLProvider: ServerInsertedHTMLProvider,
                        preinitScripts: ()=>{},
        ...
        const errorRSCPayload = await workUnitAsyncStorage.run(prerenderLegacyStore, getErrorRSCPayload, tree, ctx, reactServerErrorsByDigest.has(err.digest) ? undefined : err, errorType, // Legacy prerender recovery should include the error payload head.
        true);
        const errorServerStream = workUnitAsyncStorage.run(prerenderLegacyStore, renderFlightStream, ComponentMod, errorRSCPayload, clientModules, {
            filterStackFrame,
            onError: serverComponentsErrorHandler
        });
        try {
    ...
    return {
        get isPending () {
            if (_isPending === undefined) {
                throw Object.defineProperty(new InvariantError('Expected stream state to be initialized before reading'), "__NEXT_ERROR_CODE", {
                    value: "E1281",
                    enumerable: false,
                    configurable: true
                });
