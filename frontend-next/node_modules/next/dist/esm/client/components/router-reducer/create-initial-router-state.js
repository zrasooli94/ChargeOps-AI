...
import { convertRootFlightRouterStateToRouteTree, resolveStaleAt, processRuntimePrefetchStream, segmentCacheMap, writeDynamicRenderResponseIntoCache, writePrerenderResponseIntoCache } from '../segment-cache/cache';
...
export function createInitialRouterState({ navigatedAt, initialRSCPayload, initialFlightStreamForCache, location }) {
    ...
    if (location !== null && metadataVaryPath !== null) {
        ...
        discoverKnownRoute(Date.now(), location.pathname, location.search, null, null, initialRouteTree, metadataVaryPath, initialCouldBeIntercepted, canonicalUrl, initialSupportsPerSegmentPrefetching, false // hasDynamicRewrite
        );
        ...
        if (initialSeedData !== null && initialStaleTime !== undefined) {
            if (initialStaticStageByteLength !== undefined && initialFlightStreamForCache != null) {
                ...
                Promise.resolve(initialStaticStageByteLength).then(async (byteLength)=>{
                    const staticStageResponse = await decodeStageUntilBoundary(initialFlightStreamForCache, byteLength, undefined);
                    const now = Date.now();
                    const staleAt = await resolveStaleAt(now, staticStageResponse.s);
                    writePrerenderResponseIntoCache(now, FetchStrategy.PPR, staticStageResponse.f, undefined, staticStageResponse.h, staticStageResponse.r ?? null, staleAt, initialTree, initialRenderedSearch, true, segmentCacheMap // hydration writes are bound to the shared map
                    );
                }).catch(()=>{
                ...
                resolveStaleAt(now, initialStaleTime).then((staleAt)=>{
                    writePrerenderResponseIntoCache(now, FetchStrategy.PPR, initialFlightData, undefined, initialHeadVaryParams, initialRootVaryParams ?? null, staleAt, initialTree, initialRenderedSearch, false, segmentCacheMap // hydration writes are bound to the shared map
                    );
                }).catch(()=>{
                // The static stage processing failed. Not fatal — the page
                // rendered normally, we just won't write into the cache.
        ...
        if (initialRuntimePrefetchStream != null) {
            processRuntimePrefetchStream(Date.now(), initialRuntimePrefetchStream, initialTree, initialRenderedSearch).then((processed)=>{
                if (processed !== null) {
                    writeDynamicRenderResponseIntoCache(Date.now(), FetchStrategy.PPRRuntime, processed.flightDatas, processed.buildId, processed.isResponsePartial, processed.headVaryParams, processed.rootVaryParamsIterable, processed.staleAt, processed.navigationSeed, null, segmentCacheMap // hydration writes are bound to the shared map
                    );
                }
            }).catch(()=>{
