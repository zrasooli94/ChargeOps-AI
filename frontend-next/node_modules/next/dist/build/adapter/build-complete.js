...
function getPrerenderClassification(route, routeType, response, compute, htmlSize) {
    ...
    if (routeType === undefined || response === undefined || compute === undefined) {
        throw Object.defineProperty(new _invarianterror.InvariantError(`Expected complete prerender classification for route "${route}"`), "__NEXT_ERROR_CODE", {
            value: "E1458",
            enumerable: false,
            configurable: true
        });
    ...
    if (typeof adapterMod.onBuildComplete === 'function') {
        ...
        if (config.output === 'export') {
            ...
            for(const dynamicRoute in prerenderManifest.dynamicRoutes){
                ...
                const fallbackHtmlPath = fallbackHtmlFile !== undefined ? _path.default.join(isAppPage ? appDistDir : pagesDistDir, fallbackHtmlFile) : undefined;
                    ...
                    for (const locale of config.i18n.locales){
                        const currentOutput = {
                            ...
                            fallback: fallbackHtmlFile !== undefined ? {
                                ...
                                initialStatus: undefined,
                                postponedState: undefined,
                                filePath: _path.default.join(pagesDistDir, locale, fallbackHtmlFile)
                            } : undefined,
                            groupId: prerenderGroupId
                        };
...
async function getSharedNodeAssets({ dir, bundler, distDir, repoRoot, outputFileTracingRoot, requiredServerFiles, hasInstrumentationHook, config }) {
    ...
    if (hasInstrumentationHook) {
        const { entryHash: instrumentationEntryHash } = await loadNFT(sharedNodeAssets, sharedNodeAssetsHashes, repoRoot, _path.default.join(distDir, 'server', 'instrumentation.js.nft.json'));
...
async function loadNFT(assets, assetsHashes, repoRoot, traceFilePath) {
    ...
    for(let i = 0; i < files.length; i++){
        ...
        const fileOutputPath = _path.default.relative(repoRoot, tracedFilePath);
